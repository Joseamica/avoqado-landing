...
- Snapshot `SquareHero.tsx`, `FAQ.tsx`, and `DoodleBackground.tsx` exactly from commit `3316cf1`, with only import/name changes required by `src/components/legacy/`.
...
test('conserva la homepage anterior en /legacy sin competir en buscadores', async ({ page }) => {
  ...
  await expect(page.locator('[data-story-mode]')).toHaveCount(1);
  ...
  await expect(legacy.getByText('Tu tienda, tu gym, tu estética.', { exact: false })).toBeVisible();
...
source bodies and their existing default-exported function names unchanged. The only source edit is
the FAQ dependency path:
...
- [ ] **Step 4: Run homepage regressions and build**
...
- [ ] **Step 5: Commit the isolated legacy route**

Run:

```bash
git add src/components/legacy/LegacySquareHero.tsx src/components/legacy/LegacyFAQ.tsx src/components/legacy/LegacyDoodleBackground.tsx src/pages/legacy.astro tests/e2e/home-legacy.spec.ts docs/superpowers/plans/2026-07-16-homepage-legacy-production.md
