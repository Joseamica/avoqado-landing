...
export default function AftercareScene({ scene, progress }: { scene: StoryScene; progress: MotionValue<number> }) {
  ...
  const contextOpacity = useTransform(progress, [0.2, 0.38], [0, 1]);
  const contextX = useTransform(progress, [0.2, 0.42], [14, 0]);
  const reviewOpacity = useTransform(progress, [0.32, 0.52], [0, 1]);
  const reviewY = useTransform(progress, [0.32, 0.56], [14, 0]);
  const invoiceOpacity = useTransform(progress, [0.5, 0.7], [0, 1]);
  const invoiceY = useTransform(progress, [0.5, 0.74], [14, 0]);
      ...
      <div className="story-aftercare-visual relative grid h-full min-h-0 grid-rows-[minmax(100px,0.82fr)_minmax(0,1.18fr)] content-center gap-2.5 sm:grid-cols-[38fr_62fr] sm:grid-rows-1 sm:items-center sm:gap-8 lg:gap-12">
        ...
        <div className="story-aftercare-outcomes relative grid min-h-0 gap-2 sm:gap-3">
          <motion.div
            ...
            style={{ opacity: contextOpacity, x: contextX }}
          >
          ...
          <motion.div
            ...
            style={{ opacity: invoiceOpacity, y: invoiceY }}
          >
