...
export default function PaymentScene({ scene, progress }: { scene: StoryScene; progress: MotionValue<number> }) {
  ...
  const mobilePulseX = useTransform(progress, [0, 0.2, 0.54, 0.82, 1], [0, 16, 188, 16, 0]);
  const mobilePulseY = useTransform(progress, [0, 0.2, 0.54, 0.82, 1], [0, -46, 16, -46, 0]);
  ...
  return (
    ...
    >
      <div className="relative grid h-full min-h-0 content-center gap-2.5 sm:grid-cols-[minmax(250px,0.88fr)_minmax(190px,1.12fr)] sm:items-center sm:gap-8 lg:grid-cols-[minmax(280px,44fr)_minmax(260px,56fr)] lg:gap-12">
        <motion.div
        ...
        >
          <div className="rounded-[0.85rem] bg-neutral-50 px-3.5 py-3 text-neutral-950 sm:rounded-[1.15rem] sm:px-4 sm:py-4 lg:px-5 lg:py-5">
            ...
            <div className="space-y-1.5 sm:space-y-2">
              <div className="flex items-center justify-between rounded-[0.7rem] bg-green-100 px-3 py-2 text-green-950 sm:px-3.5 sm:py-2.5">
                <span>
                  <strong className="block text-[0.7rem] font-semibold sm:text-sm">{STORY_FIXTURE.selectedMerchant}</strong>
                  <span className="mt-0.5 block text-[0.52rem] text-green-900/70 sm:text-[0.62rem]">Merchant habilitado</span>
                </span>
                <span className="grid size-6 place-items-center rounded-full bg-green-900 text-green-100 sm:size-7">
                  <Check className="size-3.5" aria-hidden="true" />
                </span>
              </div>
              <div className="flex items-center justify-between rounded-[0.7rem] border border-black/10 px-3 py-2 text-neutral-700 sm:px-3.5 sm:py-2.5">
                <span>
                  <strong className="block text-[0.7rem] font-medium sm:text-sm">{STORY_FIXTURE.alternateMerchant}</strong>
                  <span className="mt-0.5 block text-[0.52rem] text-neutral-500 sm:text-[0.62rem]">Disponible</span>
                </span>
                <span className="size-2 rounded-full border border-neutral-400" />
              </div>
            </div>

            <p className="mt-2.5 border-t border-black/8 pt-2.5 text-center text-[0.58rem] font-medium text-neutral-600 sm:mt-3.5 sm:pt-3 sm:text-[0.68rem]">
              TPV compatible · selección manual
            </p>
          </div>
        </motion.div>
          ...
          {references.map(({ label, detail, icon: Icon, active }, index) => (
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="mt-0.5 block text-[0.5rem] leading-tight text-neutral-400 sm:text-xs">{detail}</span>
              </span>
